...
> English summary. The [Korean README](README.md) is the primary document, including the full engineering changelog (54 documented find-fix cycles, plus 7 fixes that predate the changelog — see [CHANGELOG.md](CHANGELOG.md)).
...
| Documented find-fix cycles | 54 changelog entries (symptom → root cause → measured verification), plus 7 fixes that predate the changelog |

## Engineering highlights

The [changelog](CHANGELOG.md) documents 54 find-fix cycles in "symptom → root cause → measured verification" form. Selected findings:
...
The numbered changelog (#1–#54) starts at `v1.0.1` (2026-07-16), when the
