...
> English summary. The [Korean README](README.md) is the primary document, including the full engineering changelog (53 documented find-fix cycles, plus 7 fixes that predate the changelog — see [CHANGELOG.md](CHANGELOG.md)).
...
| Documented find-fix cycles | 53 changelog entries (symptom → root cause → measured verification), plus 7 fixes that predate the changelog |

## Engineering highlights

The [changelog](CHANGELOG.md) documents 53 find-fix cycles in "symptom → root cause → measured verification" form. Selected findings:
