...
## 🔄 코드 업데이트 (재배포 — 다운타임 수 초~수십 초)

> ⚠️ 이 절차는 **무중단이 아닙니다.** `fastapi` 컨테이너가 하나뿐이라 3번 단계에서
> 기존 컨테이너를 내리고 새 컨테이너를 올리는 동안 요청이 끊깁니다 — 컨테이너 교체 +
> uvicorn 기동(모듈 import, DB 풀·Qdrant 클라이언트 생성, 스케줄러 시작)까지 대략
> 수 초~수십 초. 이미지 빌드(2번)는 기존 컨테이너가 살아 있는 동안 진행되므로 다운타임에
> 포함되지 않습니다. "DB·Qdrant 컨테이너는 그대로 둔다"는 뜻이지 요청 관점의 무중단이
> 아닙니다. 무중단이 필요하면 앞단 프록시 + 백엔드 컨테이너 2개 + 헬스체크 기반 교체가
> 필요합니다 (CHANGELOG #52 참고).
...
# 2. FastAPI만 재빌드 (기존 컨테이너는 아직 살아 있음 — 다운타임 없음)
docker-compose build fastapi

# 3. 컨테이너 교체 (이 구간에 다운타임 발생 — 기존 컨테이너 stop → 새 컨테이너 start → uvicorn 기동)
