...
**문제** (사용자 테스트 중 발견): "한 층에 소화전 또는 소화기 몇 개 있어야돼?" 질문의 웹 fallback 답변이 조문을 인용했는데 참고법령 배지가 하나도 안 떴습니다. 원인은 GPT가 인용을 `[소방시설 설치 및 관리에 관한 법률 제8조]`처럼 **조문 번호까지 대괄호 안에 넣는 세 번째 표기 변형**으로 썼기 때문 — `_extract_web_citations`는 `[법령명] 제X조`(기본형)와 `**법령명** 제X조`(#12에서 추가한 볼드형)만 알고 있어 추출 0건 → `source` 청크 미발행. #12와 같은 버그 클래스의 새 변형입니다.
