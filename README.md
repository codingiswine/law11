...
| 동시 접속 검증 | 20명 동시 요청 무실패 확인 (설계 목표 10명의 2배, 부하테스트 상세는 하단 참고) |
...
| Context Recall | 0.64 |
| Answer Relevancy | 측정 불가* |

> \* ragas 0.1.x의 Answer Relevancy는 한국어 답변을 일관되게 0으로 분류하는 언어 비호환 문제가 있어 제외했습니다 (동일 설정에서 영어 문장은 1.0, 한국어 문장은 0.0으로 채점됨을 확인).
> LLM-judge 지표는 영어 프롬프트 기반이라 한국어 답변에는 전반적으로 보수적으로 채점되는 경향이 있으며, 절대값보다는 파이프라인 변경 전후의 **상대 비교(회귀 감지)** 용도로 사용합니다 (`--compare` 모드, 5% 이상 하락 시 exit 1).
...
| `GOOGLE_SEARCH_API_KEY` | - | Google 웹 검색 |
| `NAVER_CLIENT_ID` | - | Naver 뉴스/블로그 검색 |
| `NAVER_CLIENT_SECRET` | - | Naver 뉴스/블로그 검색 |
